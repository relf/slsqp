...
unsafe extern "C" fn length_work(
    ...
    *LEN_W = (3 as libc::c_int * N1 + M) * (N1 + 1 as libc::c_int)
        + (N1 - MEQ + 1 as libc::c_int) * (MINEQ + 2 as libc::c_int)
        + 2 as libc::c_int * MINEQ + (N1 + MINEQ) * (N1 - MEQ) + 2 as libc::c_int * MEQ
        + N1 + (N + 1 as libc::c_int) * N / 2 as libc::c_int + 2 as libc::c_int * M
        + 3 as libc::c_int * N + 3 as libc::c_int * N1 + 1 as libc::c_int;
    *LEN_JW = MINEQ;
}
#[no_mangle]
pub unsafe extern "C" fn nlopt_slsqp(
    ...
    c = cgrad
        .offset(
            (mpi1 as libc::c_uint)
                .wrapping_mul(n.wrapping_add(1 as libc::c_int as libc::c_uint)) as isize,
        );
                    ...
                    while i < p {
                        ...
                        if nlopt_stop_forced(stop) != 0 {
                            ...
                            while k < (*h.offset(i as isize)).m {
                                infeasibility_cur = if infeasibility_cur
